...
export function MaxesForm({
  ...
  const [values, setValues] = useState<Record<string, string>>(() => {
    ...
    for (const g of groups) {
      for (const f of g.fields) init[f.key] = f.value === "" ? "" : String(f.value);
    }
    return init;
  });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const entries = Object.entries(values).map(([exerciseKey, v]) => ({
      exerciseKey,
      value: Number(v),
    }));
    setPending(true);
                ...
                <span className="flex items-center justify-between gap-3">
                  ...
                  <span className="flex items-center gap-2">
                    <input
                      ...
                      className="tap w-36 rounded-lg border border-[var(--line)] bg-[var(--bg-elev)] px-3 text-right text-3xl font-black"
                    />
