...
function madcow(): SeedProgram {
  ...
  const weeklyTop1rm = (weekIndex: number) => MADCOW_ESTIMATED_5RM * Math.pow(MADCOW_WEEKLY_FACTOR, weekIndex);
  const pctOfTop = (percentOfTop: number, top1rmFrac: number) =>
    Number((percentOfTop * top1rmFrac).toFixed(2));
  const rampOfTop = (
    percentsOfTop: number[],
    top1rmFrac: number,
    restSec: number,
  ) =>
    sets(
      percentsOfTop.map((p) => pctOfTop(p, top1rmFrac)),
      5,
      "1rm",
      { restSec },
    );
  const fridayWork = (top1rmFrac: number, nextTop1rmFrac: number, restSec: number): SeedSet[] => {
    const ramp = rampOfTop([50, 62.5, 75, 87.5], top1rmFrac, restSec);
    return [
      ...ramp,
      {
        setNumber: 5,
        percentBase: "1rm",
        percent: pctOfTop(100, nextTop1rmFrac),
        reps: 3,
        restSec: 180,
      ...
      },
      {
        setNumber: 6,
        percentBase: "1rm",
        percent: pctOfTop(75, top1rmFrac),
        reps: 8,
        restSec: 120,
        noteKo: "백오프 · 월요일 3세트",
      },
  ...
  return {
    ...
    weeks: [0, 1, 2, 3].map((wi) => {
      const top = weeklyTop1rm(wi);
      const nextTop = weeklyTop1rm(wi + 1);
      return {
        weekNumber: wi + 1,
        nameKo: `${wi + 1}주차`,
        ...
        days: [
          {
            ...
            nameKo: "월요일 — 헤비 5×5",
            exercises: [
              { exerciseKey: "squat", role: "main", sets: rampOfTop([50, 62.5, 75, 87.5, 100], top, 180) },
              { exerciseKey: "bench", role: "main", sets: rampOfTop([50, 62.5, 75, 87.5, 100], top, 150) },
              {
                exerciseKey: "barbell_row",
                role: "main",
                notesKo: "시트 톱은 0.7×벤치 1RM ×0.87. 앱은 로우 시작중량/1RM을 씁니다.",
                sets: rampOfTop([50, 62.5, 75, 87.5, 100], top, 120),
              },
            ],
          },
          {
            ...
            exercises: [
              {
                ...
                sets: rampOfTop([50, 62.5, 75, 75], top, 150),
              },
              { exerciseKey: "ohp", role: "main", sets: rampOfTop([62.5, 75, 87.5, 100], top, 150) },
              { exerciseKey: "deadlift", role: "main", sets: rampOfTop([62.5, 75, 87.5, 100], top, 180) },
            ],
          },
          {
            dayNumber: 3,
            nameKo: "금요일 — 트리플",
            exercises: [
              { exerciseKey: "squat", role: "main", sets: fridayWork(top, nextTop, 180) },
              { exerciseKey: "bench", role: "main", sets: fridayWork(top, nextTop, 150) },
              { exerciseKey: "barbell_row", role: "main", sets: fridayWork(top, nextTop, 120) },
            ],
          },
        ],
      };
