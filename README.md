...
3. **Container Manager → 프로젝트 → 생성**
   ...
   - NAS GHCR 배포는 `docker-compose.nas.yml` + `./deploy-pull.sh` (호스트 `/volume1/docker/strength-lab/data` 바인드). **pull does not wipe user data.** **pull does not wipe logs.** 장비 제휴 URL은 `$DATA_DIR/gear-affiliates.json` (`DEPLOY-CI.md`).
   - 빌드 후 시작
4. 또는 SSH:
