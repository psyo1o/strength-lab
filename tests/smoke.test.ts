...
describe("korean exercise tips", () => {
  ...
  it("treats imageUrl videoUrl credit as optional and prefers local /exercises/{id}.webp", () => {
    ...
    expect(tipFor("kipping_pull_up")?.youtubeCredit).toBe("CrossFit");
...
describe("mobile UX P0", () => {
  it("opens tip YouTube externally and keeps home to three cards", () => {
    ...
    expect(sheet).toMatch(/TipVideoButtons/);
    expect(sheet).toMatch(/youtubeUrl=\{tip\.youtubeUrl\}/);
    ...
    expect(maxes).toMatch(/WOD 처방/);
    expect(maxes).toMatch(/1RM이 아닙니다/);
    expect(maxes).not.toMatch(/내 장비/);
...
describe("P1 programs", () => {
  it("loads each P1 program week 1 without platePlan", () => {
    ...
    const friMain = fri.exercises.find((e: { role: string }) => e.role === "main");
    expect(friMain.notesKo).toMatch(/to_10RM/);
    expect(friMain.sets.map((s: { percent: number; reps: number }) => [s.percent, s.reps])).toEqual([
      [50, 10],
      [60, 10],
      [70, 10],
      [80, 10],
    ...
    expect(friMain.sets[friMain.sets.length - 1].amrap).toBe(true);
    ...
    expect(raw.programs.find((p: { id: string }) => p.id === "catalyst").descriptionKo).toMatch(/진행 가능/);
