...
| madcow-5x5 | 부모 1주차: 추정 5RM 0.87×1RM, 월 50–100% 톱, 수 스쿼트 4×5(3세트 반복), 금 트리플=다음 주 톱 + 백오프 75%. 매주 ×1.025. | 진행 가능 |
...
- P0 `starting-strength` gzip: 큐 텍스트에 `$<n/` 오염. `printf | base64 -d | gunzip` 실패. **주를 덮어쓰지 않음** (기존 OHP 3×5 유지).
